...
public class TakeOutPawnAction implements Action {
    ...
    public boolean isPossible(Board board, Player player, int roll) {
        if (roll != 6 || board.getPlayerPawns(player).getActivePawns().stream()
                .anyMatch(pawn -> pawn.getPosition() == player.startPosition())
        ) {
            return false;
        }
        ...
        if (isPossible(board, player, roll)) {
            ...
            for (Player currentPlayer : otherPlayers) {
                Optional<Pawn> otherPawn = board.getPlayerPawns(currentPlayer).getActivePawns().stream()
                        .filter(pawn -> player.startPosition() == pawn.getPosition())
                        .findFirst();
